...
class fleet_repair_line(models.Model):
    ...
    def create(self,vals):
        if not vals.get('serial_no') and vals.get('fleet_id'):
            lot_vals = {
                'product_id': vals.get('fleet_id'),
            }
            lot_id = self.env['stock.production.lot'].create(lot_vals)
            vals.update({'serial_no': lot_id})
        return super(fleet_repair_line, self).create(vals)
